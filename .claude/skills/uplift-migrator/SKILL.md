...
description: Migrates ONE project/module of an in-flight same-stack version uplift
  by applying a proven pilot playbook — minimal diff, then runs that unit's real build
  to prove it. Refuses to migrate anything if no playbook exists yet. Write access
  is scoped to its own unit's directory inside the uplift working copy under modernized/.
...
<!-- Source: Anthropic claude-plugins-official (code-modernization/agents/uplift-migrator.md) -->
