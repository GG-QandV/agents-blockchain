//! M7a-sui test matrix: same invariants as EVM version + Sui specifics.
use mu_connect::sui::*;
...
fn object_locked_is_unknown_not_failed() {
    // Sui analog of RISK-M7-1: equivocation → Unknown, reserve is held
...
fn node_builds_wrong_recipient_rejected_before_sign() {
    // RISK-M7-5: build-node substituted a different recipient → dry-run self-check catches it
...
fn signing_digest_deterministic_and_intent_prefixed() {
    ...
    assert_eq!(a, b);
    // intent affects: different payload → different digest
...
fn signature_serialization_format_p256() {
    ...
    // verifiable by standard P-256 (RFC 6979 → determinism verified in M4)
